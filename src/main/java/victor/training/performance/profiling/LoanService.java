...
//  fix: release the connection faster back to the pool
  public LoanApplicationDto getLoanApplication(Long loanId) {
    log.info("Start");
    List<CommentDto> comments = commentsApiClient.fetchComments(loanId); // takes ±40ms in prod //32% of time
    LoanApplication loanApplication = loanApplicationRepo.findByIdLoadingSteps(loanId);//57% of time
...
//    synchronized (recentLoanStatusQueried) { // c) best practice: synchronize on a private final field
      // we are using synchronized to avoid
